...
class TestProcessFrame(TestCase):
    def test_process_frame_general_blue(self):
        ...
        for x in range(300, 400):
            self.assertEqual(drawing.canvas[x + 1080, x + 1920].all(), np.array([255, 0, 0]).all())
    ...
    def test_process_frame_general_yellow(self):
        ...
        for y in range(300, 400):
            self.assertEqual(drawing.canvas[300 + 1080, y + 1920].all(), np.array([0, 255, 255]).all())
    ...
    def test_process_frame_general_width(self):
        ...
        for x in range(300 - 13, 300 + 13):
            for y in range(300, 400):
                self.assertEqual(drawing.canvas[x + 1080, y + 1920].all(), np.array([0, 255, 255]).all())

    def test_process_frame_no_pen(self):
        drawing = Drawing(1920, 1080)
        frame = np.zeros((1080, 1920, 3), dtype="uint8")
        drawing.process_frame(frame=frame, x=None, y=None, area=None, action=None)
        self.assertEqual(drawing.ex_action, None)
        self.assertEqual(drawing.ex_pen_pos, (0, 0))
    ...
    def test_process_frame_action_changed(self):
        ...
        self.assertEqual(drawing.ex_action, "Blue")
        self.assertEqual(drawing.ex_pen_pos, (0, 0))
...
class TestScale(TestCase):
    def test_scale_up(self):
        ...
        self.assertEqual(drawing.scale_factor, 1.25)

    def test_scale_down(self):
        drawing = Drawing(1920, 1080)
        drawing.scale(- 1080 / 10)
        self.assertEqual(drawing.scale_factor, 0.75)

    def test_scale_out_of_bounds_high(self):
        drawing = Drawing(1920, 1080)
        drawing.scale_factor = 3
        drawing.scale(10)
        self.assertEqual(drawing.scale_factor, 3)

    def test_scale_out_of_bounds_low(self):
        drawing = Drawing(1920, 1080)
        drawing.scale_factor = 0.5
        drawing.scale(-10)
        self.assertEqual(drawing.scale_factor, 0.5)

    def test_scale_near_canvas_bound(self):
        drawing = Drawing(1920, 1080)
        drawing.view_corner = (1920 * 2, 1080 * 2)
        drawing.scale(1080 / 10)
        self.assertEqual(drawing.view_corner, (3360, 1890))
